...
class MainTest extends AnyFlatSpec with Matchers {
  val partyResults: List[PartyResult] = List(
    PartyResult(11014, "C"),
    PartyResult(17803, "L"),
    PartyResult(4923, "UKIP"),
    PartyResult(2069, "LD")
  )

  "extractResults" should "a constituency result" in {
    val input: String = "Cardiff West, 11014, C, 17803, L, 4923, UKIP, 2069, LD"
    val output: ConstituencyResult =
      ConstituencyResult("Cardiff West", partyResults)
  ...
  "calculatePercentage" should "calculate the share of the vote as a percentage of all the votes cast" in {
    val input: List[PartyResult] =
      List(PartyResult(25, "C"), PartyResult(75, "L"))

    val output: List[PartyShare] =
      List(PartyShare("C", 25.0f), PartyShare("L", 75.0f))
